...
from fidelius import BC25519, KeyMaterial, CryptoController
import base64
import unittest
...
class TestIntegration(unittest.TestCase):
    ...
    def test_sane_encryption(self):
        json_message = '{"a": "b"}'
        encryption_request = ENCRYPTION_REQUEST.copy()
        encryption_request["string_to_encrypt"] = json_message
        encryption_request_se = ENCRYPTION_REQUEST.copy()
        encryption_request_se.pop("string_to_encrypt")
        encryption_request_se["string_to_encrypt_base64"] = base64.b64encode(
            json_message.encode()
        ).decode()
        self.assertEqual(
            CryptoController().sane_encrypt(encryption_request_se),
            CryptoController().encrypt(encryption_request)
        )
    ...
    def test_decryption(self):
        self.assertEqual(
            ...
            b"Wormtail should never have been Potter cottage's secret keeper."
        )
